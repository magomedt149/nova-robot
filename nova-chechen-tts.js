(() => {
    ...
    if (!document.querySelector('#novaRamzanTtsModal')) {
      ...
      modal.innerHTML = `
        <div class="photo-studio-card" style="max-width:680px">
          ...
          <p class="photo-studio-note">Бесплатная модель Meta MMS на подключённом NOVA Worker/Colab. Платные API и кредиты не используются.</p>
          ...
          <div class="studio-actions"><button id="novaRamzanSpeak" class="studio-generate" type="button">▶ Произнести</button><button id="novaRamzanStop" class="studio-reset" type="button">■ Стоп</button></div>
  ...
  window.NovaChechenTTS = Object.freeze({
    ...
    ramzanProfile: RAMZAN, workerConfig, speak, speakRamzan: speak, stop
  });
})();
