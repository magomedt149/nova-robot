(() => {
  ...
  async function handleUserText(raw) {
    ...
    if (/(?:созд|сдел|сгенер|сочин|собер|сыграй).{0,35}(?:музык|аккомпанемент|гитар|пиан|фортеп|аккордеон|бас|синт).{0,45}(?:аккорд|bpm|бпм|темп)|(?:по|из)\s+аккорд.{0,35}(?:музык|аккомпанемент|гитар|пиан|аккордеон)|(?:chord|chords).{0,30}(?:music|guitar|piano|accordion|bass|synth)/.test(lower)) {
      performRemoteGpu(text);
      return;
    }

    if (/motion\s*\+?\s*vfx|motion studio|remote gpu|gpu render|google colab|colab|колаб|удал[её]нн.*gpu|рендер.*gpu|gpu.*рендер|запусти.*gpu|gpu.*запусти|сделай.*видео|создай.*видео|сгенерируй.*видео|сделать.*видео|создать.*видео|налож.*(?:звук|аудио|музык).*видео|(?:замен|подмен|подстав|постав).*(?:музык|звук|аудио).*видео|смеш.*(?:дорож|аудио|музык|звук)|смикс.*(?:дорож|аудио|музык|звук)|(?:музык|оригинал|звук видео|дорожк).*(?:громк|тише|громче|\d{1,3}\s*%)|плавн.*(?:появ|затух|исчез).*(?:музык|звук)?|(?:fade\s*(?:in|out)).*(?:music|audio)?|экспорт.*mp4|сохран.*mp4|готов.*mp4|make.*video|create.*video|generate.*video|render.*video|replace.*(?:audio|music).*video|mix.*(?:audio|track|music)|export.*mp4|save.*mp4/.test(lower)) {
