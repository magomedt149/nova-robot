...
Если старая сессия действительно уничтожена Google, NOVA не теряет параметры render job: после появления нового worker она сначала пытается восстановить checkpoint из Google Drive (когда Drive уже был смонтирован), а затем автоматически пересоздаёт job и повторно отправляет сохранённый локальный исходник. Для файлов до примерно 600 MB включён best-effort локальный recovery cache; для более крупных файлов job/settings сохраняются, но после полного уничтожения Colab и перезапуска страницы может потребоваться один раз снова выбрать тот же исходный файл.

Когда пользователь возвращается из Colab, NOVA также пытается прочитать новый `NOVA CONNECT CODE` из буфера обмена, если браузер уже разрешает clipboard access. Если iOS требует жест пользователя, остаётся одно нажатие **«Вставить код из буфера и подключить»**.
