...
Remote GPU теперь сохраняет recovery-state в браузере и, если размер позволяет, сам исходный видеофайл в IndexedDB iPhone. При кратком сетевом обрыве NOVA использует экспоненциальные повторные проверки и продолжает текущий job, если старая Colab-сессия возвращается. Worker публикует уникальный `session_id`, поэтому новая Colab-сессия определяется автоматически.
